...
4. **右下角（高 CR + 高 Approval）在现实中不存在**——CR 13% + Approval 100% 的组合意味着"任何点进来的人都会转化，且商家一个都不拒"，这只会出现在 Trial Offer 的**账面**上（其 Reversal 率 50%~80%，实际 Approval 40%~70%，见 [`联盟角色与佣金模型.md`](./联盟角色与佣金模型.md) §7.5）。
...
| **最常见的死法** | ① **把试跑 3 天当成验证**（CPA §4.3：Approval 数据要 30~60 天才回来，你看到的"赚钱"是未扣 Reversal 的假账）；② 一开始就上全套付费工具，工具费超过收入；③ 追高派款 Trial Offer，被 40%~70% 的 Approval Rate 打穿；④ 选了自己完全不理解的品类 |
...
✅ 正确：也可能是**扣量**。区分方法：按 Sub ID 拆维度看分布——若所有流量源通过率**均匀偏低**，疑似系统性扣量；若只有某一两个源偏低，是你的流量问题（佣金模型 §10 误解 3、反作弊 §3.2 方法三）。健康区间：电商 CPS **>85%**，Lead Gen **70%~90%**，Trial 类 **40%~70%**；**<60% 且商家不给具体 Reversal 理由 = 高度疑似扣量**。
