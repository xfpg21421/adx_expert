...
`[惯例]` **健康区间**（与 [`联盟角色与佣金模型.md`](./联盟角色与佣金模型.md) §6.3 一致）：电商 CPS >85%；Lead Gen 70%~90%；**Trial Offer 20%~50%**（对应 Reversal 50%~80%）；**<60% 且对方不说明理由 = 高度疑似扣量，但此判据不适用于 Trial 类**——Trial 的健康区间本就在 60% 以下，其判据应改为「<20% 或显著低于 AM 承诺值」。
